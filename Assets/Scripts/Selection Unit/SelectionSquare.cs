// SCRIPT DE GESTION DE LA SELECTION DES UNITÉS
// Version 1.0 par Lucas Theillet le 16 septembre
// Version 2.0 par
// Version 3.0 par


// SOURCES EXTERNES    
// Calcul du triangle http://totologic.blogspot.se/2014/01/accurate-point-in-triangle-test.html
// Tutoriel https://www.youtube.com/watch?v=OCGoTiV4kbM
// Tutoriel https://www.habrador.com/tutorials/select-units-within-rectangle/
// Tutoriel https://www.youtube.com/watch?v=KAy4ugiMEt0

// LISTE DES FONCTIONS ET LEURS RÔLES
// start() > Masque le carré de selection (innactif tant que le joueur clic pas)
// update() > Appelle les fonctions de selection des unités (clic ou clic and drag) 
// SelectUnits() > Selection des unités par clic sur une, ou clic and drag sur plusieurs
// HighlightUnit() > Effet de "préselection" lors du "hover" sur une unité
// IsWithinPolygon() > Return bool si oui ou non un point est dans un polygone 
// IsWithinSquare() > Return bool si oui ou non un point est dans un triangle
// DisplaySquare() > Affichage du carré de selection
    ...
    public class SelectionSquare : MonoBehaviour
    {
        ...
        void SelectUnits()
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 2000f, 1 << 8))
                {
        ...
        //Calcul pour savoir si un point est dans un triangle (la moitié du carré de selection)
        ...
        void DisplaySquare()
        {
            ...
            int i = 0;
            //Fire ray from camera
            if (Physics.Raycast(Camera.main.ScreenPointToRay(TL), out hit, 2000f, 1 << 8))
            {
                TL = hit.point;
                i++;
            }
            if (Physics.Raycast(Camera.main.ScreenPointToRay(TR), out hit, 2000f, 1 << 8))
            {
                TR = hit.point;
                i++;
            }
            if (Physics.Raycast(Camera.main.ScreenPointToRay(BL), out hit, 2000f, 1 << 8))
            {
                BL = hit.point;
                i++;
            }
            if (Physics.Raycast(Camera.main.ScreenPointToRay(BR), out hit, 2000f, 1 << 8))
            {
                BR = hit.point;
                i++;
            }
